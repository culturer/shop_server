...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
        ...
        function setupReader() {
            ...
            if (thisS.options.preview) {
                ...
                $('#names').val(cutFileName(filename, ext, 20))
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        if (this.pending > 0) {
            ...
            //formData.append('name', thisS.toUpload[i].name);//append the first file to the form data
            //formData.append('type', thisS.toUpload[i].type);//append the first file to the form data
            //formData.append('size', thisS.toUpload[i].size);//append the first file to the form data
            ...
            var ajaxOptions = $.extend({}, {//store the request to the uploadList variable
                ...
                beforeSend: function (xhr) {
                    ...
                    if (typeof thisS.options.beforeEachUpload === 'function') {
                        ...
                        try {
                            formData.append('name', thisS.toUpload[ii].name);//append the first file to the form data
                            formData.append('type', thisS.toUpload[ii].type);//append the first file to the form data
                            formData.append('size', thisS.toUpload[ii].size);
    ...
    $.fn.ssi_uploader = function (opts) {
        var defaults = {
            ...
            onEachUpload: function (e) {
               
                var data = e.data;
                //if ("0000" == data.code) {
                    $('#file_names').val($('#file_names').val() + decodeURI(data.name) + ",")
                    $('#file_paths').val($('#file_paths').val() + decodeURI(data.path) + ",")
                    $('#paths').val($('#paths').val() + decodeURI(data.data))
                    //$('[name="Filedata"]').val($('[name="Filedata"]')[0].flie)
                    document.getElementById('file_exts').value += decodeURI(data.ext) + ",";
                    document.getElementById('file_sizes').value += decodeURI(data.size) + ",";
                    
                    var span = $('<span class="label label-info" title="' + decodeURI(data.path) + '"  ext="' + decodeURI(data.ext) + '"  size="' + decodeURI(data.size) + '" type="0">' + decodeURI(data.name) + ' <i  class="fa fa-remove" style="padding-left:5px" onclick="$(this).parent().remove();refesh()"></i></span>');
                    $('#on_upload').append(span);
                    document.getElementById('upload_type').value += "0,";
                //} 
            },
            beforeUpload: function () {
            },
            beforeEachUpload: function (msg,xhr) {
                var xx = xhr;
                var a = msg;
            },
            allowed: ['jpg', 'jpeg', 'png', 'bmp', 'gif','txt','doc','docx','xls','xlsx','pdf','ppt','pptx','zip','mp4','mp3','rar','flv','avi','mov','rmvb','wmv'],
            errorHandler: {
                method: function (msg) {
                    alert(msg);
                },
        ...
        return this.each(function () {
            ...
            if ($element.is('input[type="file"]')) {
                ...
                var ssi_uplogfad = new Ssi_upload(this, options);
                $element.data('ssi_upload', ssi_uplogfad);
            } else {
    ...
    var locale = {
        ...
        ch: {
            success: '成功',
            sucUpload: '成功上传',
            chooseFiles: '<span class="ssi-btnIn"><i class="fa fa-cloud-upload"></i>&nbsp;&nbsp;选择文件</span>',
            uploadFailed: '上传失败',
            serverError: '服务器内部出错',
            error: '错误',
            abort: '关于',
            aborted: 'Aborted',
            files: '文件列表',
            upload: '<i class="fa fa-check-circle"></i>&nbsp;&nbsp;上传',
            clear: '<i class="fa fa-times-circle"></i>&nbsp;&nbsp;取消',
            drag: '拖拽文件上传',
            sizeError: '$1 上传的文件超出 $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 文件类型不支持,如要开通请联系管理员',//$1=file extension ie(exe files are not supported)
            someErrorsOccurred: '不明错误发生!'
        },
    };
